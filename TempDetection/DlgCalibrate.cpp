...
extern unsigned int m_FreqData[30][4][12];
extern float m_TempData[30][4][12];
extern bool intTemp2;
extern int intCF;
extern bool intTemp;
extern int intRDC;
extern int IntUSRForCal;

char CalCMD[15]            ={0x00,0x44,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
CString CalAllTemp;
...
void CDlgCalibrate::OnBnClickedButtonCalibrate()
{
	...
	if (intTemp2)
	{
		...
		int CaliTempData = atoi(SetTemp);
		if (SetTemp.GetLength()!=4)
		{
			AfxMessageBox("请输入正确的数字格式！（温度为-20~150℃之间，带+-号的三位整数）");
...
void CDlgCalibrate::OnBnClickedButtonCalibrateAll()
{
	...
	if (intTemp2)
	{
		...
		int CaliTempData = atoi(SetTemp);
		if (CaliTempData>150||CaliTempData<-20)
		{
			AfxMessageBox("请输入正确的数字格式！（温度为-20~150℃之间）");
		}else{
...
void CDlgCalibrate::DataShow(float GetTemperData,int GetFreqData,int StaticTemper,int StaticFreq)
{
	...
	{
		TemperTxt.Format("%.1f",GetTemperData);
